...
MLP_user_input_variable = tf.Variable(tf.random_normal([num_users, int(layers[0]/2)], stddev = 0.001))
MLP_user_embeded = tf.nn.embedding_lookup(MLP_user_input_variable, user_input)
MLP_item_input_variable = tf.Variable(tf.random_normal([num_items, int(layers[0]/2)], stddev = 0.001))
...
for epoch in range(num_epochs):
	user_id_list, item_set, label_set = get_train_instances(train, args.num_neg)
	...
	for batch in range(len(user_id_list)//args.batch_size):
		...
		sess.run(train_step, feed_dict={user_input: user_id_batch, item_input: item_batch, label: label_batch})
	t2 = time()
	hits, ndcgs = [],[]
