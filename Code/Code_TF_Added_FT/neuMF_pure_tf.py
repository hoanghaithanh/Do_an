...
fout = open("result/NEUMF_Enhanced_{:02d}node_{:02d}fac_{:02d}neg_{}".format(layers[0],mf_dim,num_negatives,str(time())),"w")
...
user_feature = tf.placeholder(tf.float32, shape=(None,19),name="user_input_layer")
...
MF_user_input_variable = tf.Variable(tf.random_normal([num_users, mf_dim], stddev = 0.001))
MF_user_embeded = tf.nn.embedding_lookup(MF_user_input_variable, user_input)
MF_item_input_variable = tf.Variable(tf.random_normal([num_items, mf_dim], stddev = 0.001))
...
#MLP part
MLP_user_dense = tf.layers.Dense(units=layers[0]/2, name="user_dense")(user_feature)

MLP_user_input_variable = tf.Variable(tf.random_normal([num_users, int(layers[0]/4)], stddev = 0.001))
MLP_user_embeded = tf.nn.embedding_lookup(MLP_user_input_variable, user_input)
MLP_item_input_variable = tf.Variable(tf.random_normal([num_items, int(layers[0]/4)], stddev = 0.001))
MLP_item_embeded = tf.nn.embedding_lookup(MLP_item_input_variable, item_input)

MLP_predict_vector = tf.concat([tf.layers.flatten(MLP_user_dense), tf.layers.flatten(MLP_item_embeded), tf.layers.flatten(MLP_user_dense)], 1)
...
for epoch in range(num_epochs):
	...
	t1=time()
	for batch in range(len(user_id_list)//args.batch_size):
		...
		sess.run(train_step, feed_dict={user_input: user_id_batch, user_feature: np.array(user_feature_batch), item_input: item_batch, label: label_batch})
	t2 = time()
	hits, ndcgs = [],[]
	for idx in range(len(testRatings)):
		...
		predict = sess.run(prediction,feed_dict={user_input: np.array(user_id).reshape(-1,1), user_feature: users, item_input: np.array(items).reshape(-1,1)})
