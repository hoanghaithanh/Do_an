...
user_input_variable = tf.Variable(tf.random_normal([num_users, 8], stddev=0.01))
user_embeded = tf.nn.embedding_lookup(user_input_variable, user_input2)

item_input_variable = tf.Variable(tf.random_normal([num_items, 8], stddev=0.01))
item_embeded = tf.nn.embedding_lookup(item_input_variable, item_input)

feature_vector = tf.layers.Dense(units=8, name="feature", activation=tf.sigmoid)(user_input)
compose_vector = tf.multiply(tf.layers.flatten(user_embeded), tf.layers.flatten(item_embeded))
predict_vector = tf.concat([compose_vector,feature_vector],axis=1)
...
for epoch in range(epochs):
	user_in2_arr, user_in, item_set, label_set = get_train_instances(train, args.num_neg, user_arr)
	user_in2, item_in, label_in = np.array(user_in2_arr).reshape(-1,1), np.array(item_set).reshape(-1,1), np.array(label_set).reshape(-1,1)
	t1=time()
	for batch in range(len(user_in)//args.batch_size):
		user_in2_batch = user_in2[batch*batch_size:min((batch+1)*batch_size,len(user_in2))]
		user_batch = user_in[batch*batch_size:min((batch+1)*batch_size,len(user_in))]
		...
		sess.run(train_step, feed_dict={user_input2: user_in2_batch, user_input: np.array(user_batch), item_input: item_batch, label: label_batch})
	t2 = time()
	hits, ndcgs = [],[]
