...
import sys
import GMF_tf_added_ft as GMF, MLP_tf_added_ft as MLP
import argparse
...
def get_model(num_users, num_items, mf_dim=10, layers=[10], reg_layers=[0], reg_mf=0):
    ...
    feature_input = tf.keras.layers.Input(shape=(19,), dtype='float32', name = 'feature_input')
    ...
    MLP_Embedding_User = tf.keras.layers.Embedding(input_dim = num_users, output_dim = layers[0]/3, name = "mlp_embedding_user",
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_layers[0]), input_length=1)
    MLP_Embedding_Item = tf.keras.layers.Embedding(input_dim = num_items, output_dim = layers[0]/3, name = 'mlp_embedding_item',
                                  embeddings_initializer = init_normal, embeddings_regularizer = tf.keras.regularizers.l2(reg_layers[0]), input_length=1)   
    MLP_Dense_Feature = tf.keras.layers.Dense(units=layers[0]/3, kernel_regularizer=tf.keras.regularizers.l2(reg_layers[0]), input_shape=(19,), name='mlp_dense_feature')
    ...
    mlp_feature_dense = tf.keras.layers.Flatten()(MLP_Dense_Feature(feature_input))
    mlp_vector = tf.keras.layers.Concatenate()([mlp_user_latent, mlp_item_latent, mlp_feature_dense])
...
def load_pretrain_model(model, gmf_model, mlp_model, num_layers):
    ...
    model.get_layer('mlp_dense_feature').set_weights(mlp_feature_dense)
    # MLP layers
